...
describe json('/tmp/kitchen/nodes/download-centos-7.json') do
  ...
  its(['default', 'scheduled_run_list', 'day']) { should eq 27 } # from Policyfile
  ...
  its(['override', 'scheduled_run_list', 'day']) { should eq 27 } # overridden by JSON
  ...
  its(['default', 'scheduled_run_list', 'processed']) { should include('20210527-200-2100') }
